							...
							<% } %>
								<header class="header-container">
									<div class="header-section">
										...
										<div class="left-section">
											<input type="text" class="left-section_input" name="title"
												onkeyup="enterkey()">
											<input type="image" onclick="searchOn()"
												src="./resources/logo/free-icon-magnifier-2319177.png" height="17"
												width="17">
										</div>

										<div class="middle-section">
											<div class="title_div" align="center">
												<h2 class="title_Text" onclick="location.href = '<%=contextPath%>'">
													Happy Road</h2>
											</div>
										</div>
										...
										<div class="right-section">
											<% if (loginUser==null) { %>
												...
												<input type="image"
													onclick="alertify.alert('에러 메세지', '로그인 후 이용가능 합니다.');"
												...
												<% } else { %>
													<input type="image"
														onclick="location.href = 'freeboardForm.fb?cpage=1'"
														src="./resources/logo/header-게시판.png" height="17" width="17">
													<input type="image"
														onclick="location.href = 'list.fa?cpage=1&userNo=<%=loginUser.getUserNo() %>'"
														src="./resources/logo/즐겨찾기-후.png" height="17" width="17">


													<% if (loginUser.getUser_level().equals("0")) { %>
														<input type="image" class="sizelogin"
															onclick="loginUserButton()" src="./resources/logo/관리자.png"
																	height=" 17" width="17">
														
														<div class="btn-group loginUserButton" id="loginSelect"
															role="group" aria-label="Basic outlined example">
															<button type="button" onclick="location.href = 'myPage.me'"
																class="btn btn-outline-secondary"
																style="font-size: 10px; border: none;">마이페이지</button>
															<button type="button" onclick="location.href = 'logout.me'" class="btn btn-outline-secondary"
																style="font-size: 10px; border: none">로그아웃</button>
														</div>
														<% } else { %>

															<input type="image" class="sizelogin"
																onclick="loginUserButton()"
																src="./resources/logo/로그인-후.png" height="17" width="17"
																onclick="loginUserButton()">

															<div class="btn-group loginUserButton" id="loginSelect"
																role="group" aria-label="Basic outlined example">
																<button type="button"
																	onclick="location.href = 'myPage.me'"
																	class="btn btn-outline-secondary"
																	style="font-size: 10px; border: none;">마이페이지</button>
																<button type="button" onclick="location.href = 'logout.me'" class="btn btn-outline-secondary"
																	style="font-size: 10px; border: none">로그아웃</button>
								...
								<script>
									function loginUserButton() {
										if ($('#loginSelect').css('display') == 'block') {
